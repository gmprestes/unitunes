        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            foreach (SettingsProperty settingsProperty in collection)
            {
                ...
                if (!bsonDocument.Contains(settingsPropertyValue.Name))
                    this.mongoCollection.Update(query, Update.Set(settingsPropertyValue.Name, BsonNull.Value));
                else
                {
                    value = bsonDocument[settingsPropertyValue.Name];
        ...
        public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection collection)
        {
            ...
            foreach (var item in values)
            {
                ...
                if(item.Value is string)
                    element = new BsonElement(item.Key, item.Value.ToString());
                else if (item.Value is DateTime)
                    element = new BsonElement(item.Key, (DateTime)item.Value);
                else if (item.Value is bool)
                    element = new BsonElement(item.Key, (bool)item.Value);

                if (element != null && !bsonDocument.Contains(item.Key))
                    mergeDocument.Add(element);
                else
                    bsonDocument[item.Key] = element.Value;
            }
