    ...
    public class MongoDBSessionStateProvider : SessionStateStoreProviderBase
    {
            ...
            using (var memoryStream = new MemoryStream())
            {
                using (var binaryWriter = new BinaryWriter(memoryStream))
                {
                    ...
                    else
                    {
                        ...
                        //, Query.EQ("lockId", lockId.ToString())
                        var query = Query.And(Query.EQ("applicationVirtualPath", HostingEnvironment.ApplicationVirtualPath), Query.EQ("id", id));
                        var upate = Update.Set("expires", DateTime.Now.Add(this.sessionStateSection.Timeout)).Set("sessionStateItems", new BsonBinaryData(ms)).Set("locked", false).Set("sessionStateItemsCount", item.Items.Count);
        ...
        private SessionStateStoreData GetSessionStateStoreData(bool exclusive, HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actions)
        {
            ...
            try
            {
                ...
                if (bsonDocument == null)
                    bsonDocument = this.mongoCollection.FindOneAs<BsonDocument>(query);
            ...
            {
                return new SessionStateStoreData(new SessionStateItemCollection(), SessionStateUtility.GetSessionStaticObjects(context), 30);
            }
        }
