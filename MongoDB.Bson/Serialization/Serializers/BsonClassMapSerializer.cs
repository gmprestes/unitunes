...
namespace MongoDB.Bson.Serialization
{
    ...
    public class BsonClassMapSerializer<TClass> : SerializerBase<TClass>, IBsonIdProvider, IBsonDocumentSerializer, IBsonPolymorphicSerializer
    {
        ...
        private object DeserializeMemberValue(BsonDeserializationContext context, BsonMemberMap memberMap)
        {
            ...
            // Normalmente o serializador vem nulo e o metodo GetSerializer() do parametro memberMap é chamado para decidir qual serializador usar considerando o tipo de dado contido
            // no objeto bsonReader. Esta gambiarra serve para forçar o tipo de serializar a ser utilizado ignorando o tipo de dado contido no bsonReader

            // Atentar que o objeto parametro memberMap utiliza tecnicas de frozen  de seu serializador para que não seja modificado em tempo de execução, então é necessario utilizar o novo metodo criado
            // na classe BsonMemberMap ForceSetSerializer(IBsonSerializer serializer) que ignora esta condição de de frozen

            if ((bsonReader.CurrentBsonType == BsonType.Int32 || bsonReader.CurrentBsonType == BsonType.Int64 || bsonReader.CurrentBsonType == BsonType.Double) && memberMap.MemberType.FullName == "System.String")
                memberMap.ForceSetSerializer(new StringSerializer());
